...
from app_logger import (
    add_log, log_success, log_error, get_logs, clear_logs
)
                    ...
                    with st.spinner("Gerando plano de tarefas..."):
                        try:
                            ...
                            if plan_image:
                                ...
                                """
                                plan_result = image_service.analyze_image(None, prompt=prompt)
                            ...
                            with col2:
                                ...
                                try:
                                    ...
                                    if 'tarefas' in plano:
                                        ...
                                        st.write(f"**{plano.get('titulo', 'Plano de Tarefas')}**")
                                        ...
                                        for i, tarefa in enumerate(plano['tarefas']):
                                            ...
                                            with st.expander(f"{i+1}. {tarefa_titulo} ({tarefa_prio})"):
                                                st.write(tarefa.get('descricao', ''))
                                                if 'subtarefas' in tarefa and tarefa['subtarefas']:
                                                    st.write("**Subtarefas:**")
                                                    for j, subtarefa in enumerate(tarefa['subtarefas']):
                                                        st.write(f"- {subtarefa['titulo']}")

                                        # Botão para ir para a aba de tarefas
                                        if st.button("Criar Tarefas a partir deste Plano"):
    ...
    if tab_index == 2:
        ...
        def criar_tarefas_do_plano():
            if not st.session_state.last_plan:
                st.warning("Nenhum plano disponível. Gere um plano primeiro na aba 'Geração de Planejamento'.")
                return
            ...
            try:
                ...
                log_success(f"Adicionadas {len(novas_tarefas)} tarefas do plano '{plano_titulo}'")
                        ...
                        with st.expander("Subtarefas"):
                            for i, subtarefa in enumerate(tarefa['subtarefas']):
                                ...
                                with sub_cols[0]:
                                    ...
                                    if st.checkbox("", value=sub_completed, key=f"sub_{subtarefa['id']}"):
        ...
        if not st.session_state.tasks:
            st.info("Nenhuma tarefa criada. Use a aba 'Geração de Planejamento' para criar um plano e gerar tarefas.")
        else:
